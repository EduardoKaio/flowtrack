...
export default function TasksPage() {
  ...
  useEffect(() => {
    const fetchAllCategories = async () => {
      try {
        ...
        // Pré-seleciona a primeira categoria quando as categorias são carregadas
        if (resp.length > 0 && !formData.categoria) {
          setFormData(prev => ({
            ...prev,
            categoria: resp[0].name
          }));
        }
      } catch (err: any) {
  ...
  const [formData, setFormData] = useState({
    titulo: "",
    descricao: "",
    categoria: "", // Mudado de "trabalho" para "" para forçar seleção
    prioridade: "media",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!selectedCategory) {
        setError("Categoria selecionada não encontrada. Por favor, selecione novamente.");
        return;
      }
  ...
  const resetForm = () => {
    // Quando resetar, seleciona a primeira categoria disponível
  ...
  const startFormEdit = (task: Task) => {
    setEditingTask(task)
    const categoryName = categories.find(c => c.id === task.categoriaId)?.name || "";
    
    // Se não encontrar a categoria, usa a primeira disponível
    const finalCategoryName = categoryName || (categories.length > 0 ? categories[0].name : "")
                            ...
                            <div className="grid gap-2">
                              ...
                              <Select
                                ...
                                onValueChange={(value) => {
                                  ...
                                  if (error && error.includes("categoria")) {
                                    setError(null)
                                  }
                                }}
                                required
                              >
                                <SelectTrigger id="category" className={error && error.includes("categoria") ? "border-red-500" : ""}>
                              ...
                              {error && error.includes("categoria") && (
